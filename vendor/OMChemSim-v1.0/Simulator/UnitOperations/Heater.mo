...
model Heater "Model of a heater to heat a material stream"
  ...
  parameter Simulator.Files.Types.HeaterSpec spec = Simulator.Files.Types.HeaterSpec.FlowsheetEquation
    "Which quantity fixes the duty" annotation(
    Dialog(tab = "Heater Specifications", group = "Calculation Mode"));
  parameter Real Tout_spec(unit = "K") = 373.15 "Outlet temperature" annotation(
    Dialog(tab = "Heater Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.HeaterSpec.OutletTemperature));
  parameter Real Tdel_spec(unit = "K") = 10 "Temperature increase" annotation(
    Dialog(tab = "Heater Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.HeaterSpec.TemperatureIncrease));
  parameter Real Q_spec(unit = "W", displayUnit = "kW") = 1e6 "Heat added" annotation(
    Dialog(tab = "Heater Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.HeaterSpec.HeatAdded));
  parameter Real xvapout_spec(unit = "-", min = 0, max = 1) = 0.5 "Outlet vapour fraction" annotation(
    Dialog(tab = "Heater Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.HeaterSpec.OutletVaporFraction));
  //========================================================================================
