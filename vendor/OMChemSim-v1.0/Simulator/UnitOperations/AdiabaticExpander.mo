...
model AdiabaticExpander "Model of an adiabatic expander to extract energy from a vapor stream in form of pressure"
  ...
  parameter Simulator.Files.Types.ExpanderSpec spec = Simulator.Files.Types.ExpanderSpec.FlowsheetEquation
    "Which quantity fixes the duty" annotation(
    Dialog(tab = "Expander Specifications", group = "Calculation Mode"));
  parameter Real Pout_spec(unit = "Pa", displayUnit = "bar") = 101325 "Outlet pressure" annotation(
    Dialog(tab = "Expander Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.ExpanderSpec.OutletPressure));
  parameter Real Pdel_spec(unit = "Pa", displayUnit = "bar") = 1e4 "Pressure drop" annotation(
    Dialog(tab = "Expander Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.ExpanderSpec.PressureDrop));
  parameter Real Q_spec(unit = "W", displayUnit = "kW") = -1e4 "Shaft power (negative: the stream gives up enthalpy)" annotation(
    Dialog(tab = "Expander Specifications", group = "Calculation Mode",
      enable = spec == Simulator.Files.Types.ExpanderSpec.ShaftPower));
  //====================================================================================
  //Model Variables
